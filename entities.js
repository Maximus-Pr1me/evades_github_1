...
class Entity {
  ...
  update(time) {
    ...
    this.convert_velocity_To_Angle();
    this.convert_angle_To_Velocity();
  ...
  colide(boundary) {
    if(this.no_collide) {
      return; //maybe "no_collide" it collides and bounces off of the wall?
    }
    let updated = false;
    if(!boundary.wall){ //if there is no property "wall" inside boundary
      if (this.pos.x - this.radius < boundary.x) { // if x-component of ball within boundary,
        if(this.color == "#7e7cd6"&&!this.precise_movement) { //if colour is Violet-blue (Crayola) and precise_movement is switched off
          this.convert_angle_To_Velocity(); //convert angle into Velocity
        }
        this.vel.x = Math.abs(this.vel.x);
        updated = true;
      }
      if (this.pos.x + this.radius > boundary.x + boundary.w) {
        if(this.color == "#7e7cd6"&&!this.precise_movement)this.convert_angle_To_Velocity();
        this.vel.x = -Math.abs(this.vel.x);
        updated = true
      }
...
class Player {
  constructor(pos, type, speed, color, className) {
    ...
    this.addX=0;
    this.addY=0;
    this.in_X_direction=0;
    this.in_Y_direction=0;
    this.die = false;
    ...
    if (input.keys) {
      ...
      if (!this.prevSlippery||this.collides||(this.d_x == 0 && this.d_y == 0)) {
        ...
        this.mouseActive = false;
          if (input.isMouse&&!this.cent_is_moving&&!(input.keys[87] || input.keys[38]||input.keys[65] || input.keys[37]||input.keys[83] || input.keys[40]||input.keys[68] || input.keys[39])) {
            ...
            if(this.className!="Cent" || (this.className=="Cent" && this.cent_input_ready)){
              ...
              this.in_X_direction = Math.round(input.mouse.x - width / 2);
              this.in_Y_direction = Math.round(input.mouse.y - height / 2);
              this.dist = distance(new Vector(0, 0), new Vector(this.in_X_direction, this.in_Y_direction));
              if (this.dist > this.mouse_distance_full_strenght) {
                this.in_X_direction = this.in_X_direction * (this.mouse_distance_full_strenght / this.dist);
                this.in_Y_direction = this.in_Y_direction * (this.mouse_distance_full_strenght / this.dist);
              }
              this.mouse_angle = Math.atan2(this.in_Y_direction,this.in_X_direction);
              this.input_angle = this.mouse_angle;
              this.mouse_distance = Math.min(this.mouse_distance_full_strenght,Math.sqrt(this.in_X_direction**2+this.in_Y_direction**2))
            ...
            if(this.className!="Cent"){this.vel.x = this.in_X_direction * this.speed / this.mouse_distance_full_strenght;
            this.addX = this.in_X_direction * this.speedAdditioner/this.mouse_distance_full_strenght;
            this.addY = this.in_Y_direction * this.speedAdditioner/this.mouse_distance_full_strenght;
            if(!this.magnet||this.magnet&&this.safeZone){if(this.vertSpeed==-1){this.vel.y = this.in_Y_direction * this.speed / this.mouse_distance_full_strenght;}else{this.vel.y = this.in_Y_direction * this.vertSpeed / this.mouse_distance_full_strenght;}} 
            }
        } else if (!this.cent_is_moving){
            this.in_Y_direction = 0; this.in_X_direction = 0;
            this.moving = false;
            ...
            if (input.keys[83] || input.keys[40]) {
              this.in_Y_direction=1;//if(this.className!="Cent"){if(!this.magnet||this.magnet&&this.safeZone){if(this.vertSpeed==-1){this.d_y = this.speed;this.addY = this.speedAdditioner}else{this.d_y = this.vertSpeed}}}else{this.dirY=1}
            }
            if (input.keys[87] || input.keys[38]) {
              this.in_Y_direction=-1;//if(this.className!="Cent"){if(!this.magnet||this.magnet&&this.safeZone){if(this.vertSpeed==-1){this.d_y = -this.speed;this.addY = -this.speedAdditioner}else{this.d_y= -this.vertSpeed}}}else{this.dirY=-1}
            }
            if (input.keys[65] || input.keys[37]) {
              this.in_X_direction=-1;//if(this.className!="Cent"){this.d_x = -this.speed;this.addX = -this.speedAdditioner}else{this.dirX=-1}
            }
        ...
        if(this.cent_distance){
          this.d_x = this.in_X_direction * this.cent_distance;
          this.d_y = this.in_Y_direction * this.cent_distance;
        }
        else if(this.moving&&!input.isMouse&&this.className!="Cent") {
          this.d_x = this.distance_movement * this.in_X_direction;
          this.d_y = this.distance_movement * this.in_Y_direction;
        }
  ...
  abilities(time, area, offset) {
    if (this.firstAbility && this.firstAbilityCooldown == 0) {
      ...
      if(this.stomp&&this.energy>=10){
        for (var i in area.entities) { //for every entity in the area
          for (var j in area.entities[i]) { //for every 
            var entity = area.entities[i][j];
...
class Dasher extends Enemy {
  ...
  compute_speed(){
    this.speed = (this.time_since_last_dash < this.time_between_dashes && this.time_dashing == 0 && this.time_preparing == 0) ? 0 : (this.time_dashing == 0) ? this.prepare_speed : this.base_speed
    //(this.time_preparing>0) ? this.prepare_speed : this.base_speed
    this.convert_angle_To_Velocity();
    this.oldAngle = this.angle;
  }
  behavior(time, area, offset, players) {
...
class Homing extends Enemy {
  constructor(pos, radius, speed, angle) { //take position, radius, speed and angle
  ...
  behavior(time, area, offset, players) {
    ...
    var index;
    for (var i in players) {
      if(!players[i].safeZone&&!players[i].night&&!players[i].god) {
...
class FrostGiant extends Enemy {
  constructor(pos, radius, speed = 0, angle = Math.floor(Math.random()*360), direction = 1, turn_speed = 2, shot_interval = 200, cone_angle = 45, pause_interval = 0, pause_duration = 0, turn_acceleration = 0, shot_acceleration = 0, pattern, immune = true, projectile_duration = 4000,projectile_radius = 10,projectile_speed = 4,precise_movement = false) {
    ...
    if(speed>0){this.convert_velocity_To_Angle();}
...
class Crumbling extends Enemy {
  ...
  behavior(time, area, offset, players) {
    ...
    if(this.collision&&this.clock>3000){
      this.collision = false;
      this.angle = this.convert_velocity_To_Angle()
      this.clock = 0;
