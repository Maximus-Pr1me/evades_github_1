...
class Game {
  ...
  update(time) {
    ...
    for (var i in this.players) {
      ...
      if (!this.players[i].wallGod && !this.players[i].no_teleport) {
        this.teleport(this.players[i]);
      }
      if(!this.players[i].wallGod) {
...
class Area {
  ...
  load() {
    ...
    for (var i in this.preset) {
      ...
      do {
        ...
        //console.log(this.preset[i].angle)
        ...
        for (var j = 0; j < count; j++) {
          ...
          if (this.preset[i].type[rand] == "fake_pumpkin") {
            enemy = new FakePumpkin(new Vector(posX, posY), 0, radius / 32, 0)
          }
  ...
  addEffect(type,pos){
    switch(type){
      case 0:
        if(!this.entities["SweetTooth"]){
          this.entities["SweetTooth"] = []
        }
